...
List<_ComposedRpcClientGetter> _resolveComposedRpcClientGetters(
  ...
  for (final importedModule in rootModule.importedModules) {
    ...
    for (final controller in importedModule.rpcCompatibleControllers) {
      ...
      _recordComposedRpcClientGetter(
        getters,
        nextGetter,
...
void _recordComposedRpcClientGetter(
  Map<String, _ComposedRpcClientGetter> getters,
  _ComposedRpcClientGetter nextGetter, {
  required InterfaceElement moduleElement,
}) {
  final getterName = nextGetter.clientGetterName;
  ...
  throw InvalidGenerationSourceError(
    'Module "${moduleElement.displayName}" resolves RPC client namespace '
    '"$getterName" from more than one source.',
